...
int i = 0;
// Define a structure to hold the size and pointer pair
struct SizePointerPair {
    size_t size;
    uint8_t *pointer;
};
struct SizePointerPair tx_buff[1]; 
...
// amin : custom memcache process function
int perform_set_wout_udp();
int perform_get_wout_udp();
...
size_t process_through_memcached(uint8_t* rx_buff_ptr) {
...
int perform_set_wout_udp(uint8_t *key, uint32_t key_len, uint8_t *val, uint32_t val_len) {

    // printf("key_len: %d\n", key_len);
    // printf("val_len: %d\n", val_len);

    // unsigned char *bytes = (unsigned char *)key;
    // printf("1First 64 bytes of tx_buff_ptr: ");
    // for (int i = 0; i < key_len; ++i) {
    //     printf("%u ", bytes[i]); // Printing in hexadecimal format
    // }
    // printf("\n");

    // bytes = (unsigned char *)val;
    // printf("2First 64 bytes of tx_buff_ptr: ");
...
int perform_get_wout_udp(uint8_t *key, uint32_t key_len, uint8_t** val, uint32_t *val_len, size_t *hit, size_t *miss) {
    // Look-up.
    uint32_t hv;
    hv = hash(key, key_len);
    item_lock(hv);
...
static void settings_init(void) {
    ...
    // AMIN
    settings.key_size = 20;
    settings.value_size = 200;
    settings.ksize_min = 30;
    settings.ksize_max = 30;
    settings.vsize_min = 200;
    settings.vsize_max = 200;
    settings.ksize_skew = 0.9;
    settings.vsize_skew = 0.9;
    settings.kMaxValSize = 1024;
    settings.dataset_size = 20000;
    settings.populate_ds_size = 10000;
    settings.use_udp = 0;
...
void process_stat_settings(ADD_STAT add_stats, void *c) {
    assert(add_stats);
    // AMIN
    APPEND_STAT("dataset_size", "%llu", (unsigned long long)settings.dataset_size);
    APPEND_STAT("key_size", "%llu", (unsigned long long)settings.key_size);
    APPEND_STAT("value_size", "%llu", (unsigned long long)settings.value_size);
    APPEND_STAT("ksize_min", "%llu", (unsigned long long)settings.ksize_min);
    APPEND_STAT("ksize_max", "%llu", (unsigned long long)settings.ksize_max);
    APPEND_STAT("vsize_min", "%llu", (unsigned long long)settings.vsize_min);
    APPEND_STAT("vsize_max", "%llu", (unsigned long long)settings.vsize_max);
    APPEND_STAT("ksize_skew", "%.2f", settings.ksize_skew);
    APPEND_STAT("vsize_skew", "%.2f", settings.vsize_skew);
    APPEND_STAT("kMaxValSize", "%llu", (unsigned long long)settings.kMaxValSize);
    APPEND_STAT("populate_ds_size", "%llu", (unsigned long long)settings.populate_ds_size);
    APPEND_STAT("use_udp", "%llu", (unsigned long long)settings.use_udp);
...
static enum try_read_result try_read_udp(conn *c) {
    ...
    if (res > 8) {
        ...
        c->rbytes = res;
        c->rcurr = c->rbuf;
        printf("just before try_read_command_udp\n");
    ...
    while (!stop) {
        ...
        case conn_read:
            if (!IS_UDP(c->transport)) {
                ...
                // UDP connections always have a static buffer. - AMIN
                printf("UDP connection - AMIN\n");
                res = try_read_udp(c);
            }
...
static void usage(void) {
    ...
    // AMIN
    printf("-dataset_size=<num>       size of the dataset in MB\n");
    printf("-populate_ds_size=<num>   size of the dataset to be populated in MB\n");
    printf("-key_size=<num>           size of the key in bytes\n");
    printf("-value_size=<num>         size of the value in bytes\n");
    printf("-ksize_min=<num>          minimum size of the key in bytes\n");
    printf("-ksize_max=<num>          maximum size of the key in bytes\n");
    printf("-vsize_min=<num>          minimum size of the value in bytes\n");
    printf("-vsize_max=<num>          maximum size of the value in bytes\n");
    printf("-ksize_skew=<num>         skewness of the key size\n");
    printf("-vsize_skew=<num>         skewness of the value size\n");
    printf("-kMaxValSize=<num>        maximum size of the value in bytes\n");
    printf("-use_udp=<num>           use udp module for now\n");
...
int main (int argc, char **argv) {
    ...
    fprintf(stderr, "selfcontained-version of memcached is ready to accept requests!\n");
    while(!stop_main_loop) {
        // for (int i = 0; i < 100000; i++){
        //     struct timespec start_time, end_time;
        //     clock_gettime(CLOCK_MONOTONIC, &start_time);
            if (settings.use_udp == 1) {
                uint8_t *rx_buff_ptr = SelfContainedUDP(settings.dataset_size, settings.key_size, settings.value_size, settings.vsize_skew,
                                        settings.vsize_min, settings.vsize_max, settings.ksize_skew, settings.ksize_min, 
                                        settings.ksize_max, settings.populate_ds_size, settings.kMaxValSize, settings.use_udp);
                if (rx_buff_ptr == NULL) {
                    fprintf(stderr, "Error in SelfContained\n");
                    return 1;
                }
            }
            else {

                SelfContained(2000000, settings.key_size, settings.value_size, settings.vsize_skew,
                                        settings.vsize_min, settings.vsize_max, settings.ksize_skew, settings.ksize_min, 
                                        settings.ksize_max, 1000000, settings.kMaxValSize, settings.use_udp, true);
                SelfContained(2000000, settings.key_size, settings.value_size, settings.vsize_skew,
                                        settings.vsize_min, settings.vsize_max, settings.ksize_skew, settings.ksize_min, 
                                        settings.ksize_max, 1000000, settings.kMaxValSize, settings.use_udp, false);
            return;
            }
        }
